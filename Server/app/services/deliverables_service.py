...
from ..models.sponsor_master_model import Eve_SponsorMasterDetail
...
#     # Check for references in CategoryWiseDetailDeliverablesMaster
#     result = await db.execute(
#         select(func.count(CategoryWiseDetailDeliverablesMaster.CatDeliverableDetailId))
#         .where(CategoryWiseDetailDeliverablesMaster.Deliverabled_Code == deliverable_id)
#     )
#     category_reference_count = result.scalar()
    
#     # Check for references in Eve_SponsorMasterDetail
#     result = await db.execute(
#         select(func.count(Eve_SponsorMasterDetail.SponsorDetailId))
#         .where(Eve_SponsorMasterDetail.Deliverabled_Code == deliverable_id)
#     )
#     sponsor_detail_reference_count = result.scalar()
    
    
#     # If there are any references in any table, prevent deletion
#     if category_reference_count > 0 or sponsor_detail_reference_count > 0 :
#         error_messages = []
#         if category_reference_count > 0:
#             error_messages.append(f"Referenced in {category_reference_count} category-wise deliverable(s)")
#         if sponsor_detail_reference_count > 0:
#             error_messages.append(f"Referenced in {sponsor_detail_reference_count} sponsor detail(s)")
        
#         return {
#             "success": False, 
#             "message": f"Cannot delete deliverable. It is {', '.join(error_messages)}."
#         }
