...
async def get_all_data_with_details(db: AsyncSession, user_id: int, ) -> List[Dict[str, Any]]:
    query = text("""
     SELECT DISTINCT 
                         ...
                         dbo.Eve_TaskHead.tran_type, tbluser_1.User_Id, tbluser_1.User_Name, dbo.Eve_TaskDetail.id, dbo.Eve_TaskDetail.userId, dbo.tbluser.User_Name AS Expr2, dbo.Eve_TaskHead.Authorised_User AS AutherizedPerson,dbo.Eve_TaskHead.TeamMasterId
...
async def create_task(db: AsyncSession, task_data: TaskCreate,ws_manager: Optional[ConnectionManager] = None):
    ...
    print("Created TaskHead with taskno:", db_task.TeamMasterId)
    ...
    if task_data.details is not None:
        for detail in task_data.details:
            ...
            if action == "add":
                ...
                db.add(db_detail)
                await db.flush()
                added_details[user_id] = db_detail.id 
