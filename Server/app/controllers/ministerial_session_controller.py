...
async def create_ministerial_session_endpoint(
    ...
    try:
        return await create_ministerial_session(db, session_data)
...
async def update_existing_ministerial_session(
    ...
    updated_session = await update_ministerial_session(db=db, session_id=session_id, session=session)
...
async def delete_existing_ministerial_session(
    ...
    success = await delete_ministerial_session(db=db, session_id=session_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Ministerial session not found"
