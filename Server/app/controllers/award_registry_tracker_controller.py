...
async def get_all_award_registry_data(
    ...
    if event_code:
        results = await get_award_registry_trackers(db, event_code)
    elif sponsor_id:
        results = await get_award_trackers_by_sponsor(db, sponsor_id)
    elif deliverable_code:
        results = await get_award_trackers_by_deliverable(db, deliverable_code)
    else:
        results = await get_award_registry_trackers(db) 

    return [AwardRegistryTracker(**dict(result)) for result in results]
