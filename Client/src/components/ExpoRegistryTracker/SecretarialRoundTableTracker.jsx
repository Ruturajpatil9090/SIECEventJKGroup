...
} from '../../services/secretarialRoundtableApi';
import {
...
function SecretarialRoundTableTracker() {
    ...
    const [secretarialRoundTableIdToDelete, setSecretarialRoundTableIdToDelete] = useState(null);
    const [formData, setFormData] = useState({
        SecretarialRoundTableId: '',
        Event_Code: '',
        SponsorMasterId: '',
        Speaker_Name: '',
        designation: '',
    ...
    const {
        ...
        isError,
        refetch
    } = useGetSecretarialRoundtablesQuery({ event_code: sessionStorage.getItem("Event_Code") });

    const {
        data: maxSecretarialRoundTableId = 0,
    ...
    const [addSecretarialRoundtable] = useAddSecretarialRoundtableMutation();
    const [updateSecretarialRoundtable] = useUpdateSecretarialRoundtableMutation();
    const [deleteSecretarialRoundtable] = useDeleteSecretarialRoundtableMutation();
    ...
    const { data: detailData, isLoading: isDetailLoading, refetch: refetchDetails } =
        useGetSecretarialRoundtableDetailsByIdQuery(selectedRecordId, {
            skip: !selectedRecordId
        });

    const handleShowPopup = async (row) => {
        setSelectedRecordId(row.SecretarialRoundTableId);
    ...
    useEffect(() => {
        if (!editId && !isMaxIdLoading && isModalOpen) {
            const nextId = maxSecretarialRoundTableId + 1;
            setFormData(prev => ({
                ...prev,
                SecretarialRoundTableId: nextId.toString()
            }));
        }
    }, [maxSecretarialRoundTableId, isMaxIdLoading, editId, isModalOpen]);
    ...
    const columns = [
        ...
        },
        {
            ...
            cellRenderer: (row) => row.Invitation_Sent === 'Y' ? 'Yes' : 'No'
        },
        {
            header: 'Approval Received',
            accessor: 'Approval_Received',
            cellRenderer: (row) => row.Approval_Received === 'Y' ? 'Yes' : 'No'
        },
        {
            header: 'Action',
            accessor: 'action',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                SecretarialRoundTable_Bio: formData.SecretarialRoundTable_Bio,
            ...
            if (editId) {
                await updateSecretarialRoundtable({
                    id: Number(editId),
                    ...payload
                }).unwrap();
                showNotification('Secretarial RoundTable updated successfully!');
            } else {
                await addSecretarialRoundtable({
                    SecretarialRoundTableId: Number(formData.SecretarialRoundTableId),
                    ...payload
                }).unwrap();
                showNotification('Secretarial RoundTable added successfully!');
            }
            ...
            console.error('Failed to save secretarial roundtable:', error);
            showNotification('Failed to save secretarial roundtable!', 'error');
        }
    };

    const handleEdit = (row) => {
        setFormData({
            SecretarialRoundTableId: row.SecretarialRoundTableId,
            ...
            SecretarialRoundTable_Bio: row.SecretarialRoundTable_Bio || '',
            Speaking_Date: row.Speaking_Date ? row.Speaking_Date : getCurrentDate(),
            Track: row.Track || '',
            Invitation_Sent: row.Invitation_Sent || 'N',
            Approval_Received: row.Approval_Received || 'N'
        });
        setEditId(row.SecretarialRoundTableId);
        setIsModalOpen(true);
    };

    const openDeleteConfirmModal = (id) => {
        setSecretarialRoundTableIdToDelete(id);
        setIsConfirmDeleteModalOpen(true);
    };

    const confirmDelete = async () => {
        if (secretarialRoundTableIdToDelete) {
            try {
                await deleteSecretarialRoundtable(Number(secretarialRoundTableIdToDelete)).unwrap();
                showNotification('Secretarial RoundTable deleted successfully!');
                refetch();
            } catch (error) {
                console.error('Failed to delete secretarial roundtable:', error);
                showNotification('Failed to delete secretarial roundtable!', 'error');
            } finally {
                setIsConfirmDeleteModalOpen(false);
                setSecretarialRoundTableIdToDelete(null);
            }
        }
    };
    ...
    return (
        <>
            ...
            <Modal
                ...
                title={editId ? 'Edit Secretarial RoundTable' : 'Add New Secretarial RoundTable'}
                width="800px"
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                Secretarial RoundTable ID
                            </label>
                            <input
                                type="text"
                                name="SecretarialRoundTableId"
                                value={formData.SecretarialRoundTableId}
                    ...
                    <div>
                        ...
                        <textarea
                            name="SecretarialRoundTable_Bio"
                            value={formData.SecretarialRoundTable_Bio}
                            onChange={handleInputChange}
                            rows={3}
                            autoComplete='off'
                            data-gramm="false"
            ...
            >
                <div className="p-4">
                    <p className="text-gray-700 text-lg mb-4">
                        Are you sure you want to delete this secretarial roundtable record?
                    </p>
            ...
            <SecretarialRoundTableDetailView
...
export default SecretarialRoundTableTracker;