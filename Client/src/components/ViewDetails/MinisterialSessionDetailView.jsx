...
function MinisterialSessionDetailView({ isOpen, onClose, details, mainData, sponsors = [] }) {
  ...
  if (!sessionDetails) {
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="Ministerial Session Details" size="lg">
        <div className="p-6 text-center text-gray-500">
          <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-gray-100">
            <DocumentTextIcon className="h-6 w-6 text-gray-400" />
          </div>
          <h3 className="mt-2 text-sm font-medium text-gray-900">No details available</h3>
          <p className="mt-1 text-sm text-gray-500">There are no details for this ministerial session.</p>
        </div>
      </Modal>
    );
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Ministerial Session Details" size="2xl">
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border border-blue-100">
          <div className="flex justify-between items-start mb-4">
            <h3 className="font-semibold text-lg text-gray-800 flex items-center">
              <BuildingOfficeIcon className="h-5 w-5 mr-2 text-blue-600" />
              Ministerial Session Information
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-md font-semibold text-gray-800">{sessionDetails.MinisterialSessionId}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${sessionDetails.Invitation_Sent === 'Y'
                ? 'bg-green-100 text-green-800'
                : 'bg-yellow-100 text-yellow-800'
                }`}>
                {sessionDetails.Invitation_Sent === 'Y' ? 'Sent' : 'Not Sent'}
                {sessionDetails.Invitation_Sent === 'Y' ? (
        ...
        {activeTab === 'details' ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">{sessionDetails.Track || 'Not specified'}</p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="prose prose-sm max-w-none">
              {sessionDetails.MinisterialSession_Bio ? (
...
export default MinisterialSessionDetailView;