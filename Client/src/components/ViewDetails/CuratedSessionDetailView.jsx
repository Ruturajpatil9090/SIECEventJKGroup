...
function CuratedSessionDetailView({ isOpen, onClose, details, mainData }) {
  ...
  if (!sessionDetails) {
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="Curated Session Details" size="lg">
        <div className="p-6 text-center text-gray-500">
          <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-gray-100">
            <DocumentTextIcon className="h-6 w-6 text-gray-400" />
          </div>
          <h3 className="mt-2 text-sm font-medium text-gray-900">No details available</h3>
          <p className="mt-1 text-sm text-gray-500">There are no details for this curated session.</p>
        </div>
      </Modal>
    );
  }
        ...
        <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 rounded-lg border border-purple-100">
          <div className="flex justify-between items-start mb-4">
            <h3 className="font-semibold text-lg text-gray-800 flex items-center">
              <MicrophoneIcon className="h-5 w-5 mr-2 text-purple-600" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-md font-semibold text-gray-800">{sessionDetails.CuratedSessionId}</p>
            </div>
            <div>
...
export default CuratedSessionDetailView;